...
echo $nombre_pago=$_POST['nombre_pago'];
...
if (md5($str) == $pase_encr)
  {
  $query="INSERT INTO recibo (fecha_hora_pago, tipo_pago , id_alumno, id_asignacion_grado_seccion, nombre_persona_q_paga, codigo_recibo, total) VALUES('$dia_hora', '$nombre_pago' ,'$id_alumno','$ciclo','$persona_paga','$codigo_unico_recibo','$total')";
	        if ($conexion->query($query) === TRUE) {
			    echo "Ingresado Correctamente";
			    echo'<script> swal("Inscripcion Ingresada Correctamente") </script';
			    $url = "pago_varios.php?id=".$id_alumno."&id_grado=".$ciclo."&anio=".$anio;
				header('Location: '.$url);
			} else {
	...
	$url = "pago_varios.php?id=".$id_alumno."&id_grado=".$ciclo."&anio=".$anio."&err=5";
	header('Location: '.$url);
die();
